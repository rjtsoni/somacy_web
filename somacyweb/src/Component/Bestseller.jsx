        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3 lg:gap-4">
          {bestsellers.map((product) => (
            <div
              ...
              className="bg-white rounded-[12px] sm:rounded-[16px] p-2 lg:p-3 shadow-sm border border-slate-100/50 flex flex-col hover:shadow-md transition-shadow"
            >
              <div
                className={`${product.imageBg} rounded-[8px] sm:rounded-[12px] h-20 sm:h-24 lg:h-28 relative mb-1.5 sm:mb-3 flex items-center justify-center overflow-hidden shrink-0`}
              >
                  ...
                  <div className="absolute top-1 left-1 lg:top-2 lg:left-2 bg-[#B69156] text-white px-1 sm:px-1.5 py-0.5 rounded-[4px] sm:rounded-full flex items-center gap-0.5 sm:gap-1 shadow-sm">
                    <div className="w-1.5 h-1.5 sm:w-2 lg:w-2.5 sm:h-2 lg:h-2.5 rounded-full border border-white/50 flex items-center justify-center">
                      <Check
                        size={9}
                        strokeWidth={4}
                        className="w-[5px] h-[5px] sm:w-[6px] sm:h-[6px] lg:w-[7px] lg:h-[7px]"
                      />
                    </div>
                    <span className="text-[5px] sm:text-[6px] lg:text-[7px] font-bold uppercase tracking-wider hidden sm:inline">
                      RGHS Verified
                    </span>
                    <span className="text-[5px] font-bold uppercase tracking-wider sm:hidden">
                      RGHS
                    </span>
                  </div>
                )}
