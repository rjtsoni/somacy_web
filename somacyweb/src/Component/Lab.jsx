        ...
        <section className="py-6 bg-[#F8FAFC]">
            <div className="w-full px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col md:flex-row md:justify-between md:items-end mb-4 sm:mb-6 gap-4">
                    <div>
                        <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Lab Tests by Organ/Condition</h2>
                        <p className="text-slate-500 mt-2 font-medium">Book tests specifically for your symptoms or health checks</p>
                    </div>
                    <button
                        onClick={() => navigate("/lab-test/lab", { state: { from: 'lab' } })}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6">
                    {CATEGORIES.map((item) => {
                        ...
                        return (
                            <div
                                key={item.id}
                                onClick={() => navigate(`/lab-test/${item.name.toLowerCase()}`, { state: { from: 'lab' } })}
                            ...
                            >
                                ...
                                <h3 className={`mt-4 font-bold ${isSelected ? 'text-blue-600' : 'text-gray-800'}`}>{item.name}</h3>
